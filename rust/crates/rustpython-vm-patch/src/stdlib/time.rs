...
mod time {
    ...
    // WASI doesn't have CLOCK_PROCESS_CPUTIME_ID in libc, so use a fallback
    #[cfg(all(target_arch = "wasm32", target_os = "wasi"))]
    pub(super) fn get_process_time(_vm: &VirtualMachine) -> PyResult<Duration> {
        // WASI doesn't support process CPU time; return zero
        Ok(Duration::new(0, 0))
    }
