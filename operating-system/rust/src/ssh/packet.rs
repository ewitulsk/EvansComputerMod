...
pub fn encode_packet_with_block_size(payload: &[u8], block_size: usize) -> Vec<u8> {
    ...
    // padding_length (1 byte) + payload + padding must be multiple of block_size
    // minimum padding = 4
    ...
    let packet_length = 1 + payload.len() + padding_len; // padding_length + payload + padding
...
pub fn decode_packet(data: &[u8]) -> Option<(Vec<u8>, usize)> {
    if data.len() < 5 {
        return None; // Need at least packet_length (4) + padding_length (1)
    }

    let packet_length = u32::from_be_bytes([data[0], data[1], data[2], data[3]]) as usize;

    if packet_length < 1 || packet_length > 35000 {
        return None; // Invalid or too large
    }

    let total = 4 + packet_length; // 4 bytes for packet_length field + packet contents
    if data.len() < total {
